...
export function useTodo() {
    ...
    const addStaticTodo = (e) => {
        ...
        if (input) {
            const newTodo = {
                account: {
                    idx: parseInt(todos[todos.length - 1].account.idx) + 1, content: input, marked: false
                }
            }
    ...
    const markStaticTodo = (todoID) => {
        setTodos(todos.map(todo => {
            ...
            if (todo.account.idx === todoID) {
                ...
                return {
                    account: {
                        idx: todo.account.idx, content: todo.account.content, marked: !todo.account.marked
                    }
                }
            }
